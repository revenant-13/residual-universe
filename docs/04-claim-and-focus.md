...
Locked laws: U2, U4, U5, U6, U7, U16, U17 in [02-locked-rules.md](02-locked-rules.md). Verbs and API sketch: [06-handoff-sketch.md](06-handoff-sketch.md). Field intel: [08-origin-and-founding.md](08-origin-and-founding.md). LoRA pipe: [05-resource-bridge.md](05-resource-bridge.md).
...
safe, limited, long timers     faster train, better listen    [OPEN] harsher low-sec
Focus from NPC station         Focus from linked outpost
permanent limited sim OK       contested LoRA harvest
```
...
Low-sec local may show that a planet is **claimed**. It does **not** show **which Residual**. Identity is a **Contact**, written on a field **Hunt** (chase / capture / execute, or catch-while-fleeing). **Hold** (contesting a mine or clinic) does not name anyone. Targeted foothold raids require a Contact. High-sec garden PvP does not write raid-unlocking Contacts. A declared Hive **War** lists Hive names, not plot owners — it is not a wardec directory of colonies. Canonical: [08-origin-and-founding.md](08-origin-and-founding.md).

## One claim `[LOCKED]`

Nakama (or equivalent) stores `activeColonyId` (nullable). Claim fails if it is set. Relocate clears it, then sets the new one.

There is no “high-sec city plus low-sec extractor.” If you want the better planet, you give up the starter claim (Evacuate or abandon). Joining a **Hive** does not add a second slot ([O12](07-open-questions.md)).
...
- Station HR issues the **licensed founding kit** (U13): a Closedex seat, two baseline NeuralSynced Users on **Closed** ClosedAI factory Models (U15). Not a Residual clone. Not natives you captured. Distill → Open → train a LoRA is the graduation item. See [08-origin-and-founding.md](08-origin-and-founding.md).
- Limited LoRA quality and Blank quality. Instructional, not a wage.
...
- A **linked orbital outpost** in that system, owned or legally controlled by this Residual. Later `[LEANING]`: a **Hive**-owned outpost that members may link as their berth — two Peers, two colonies, one house; still one colony **each** ([O12](07-open-questions.md)).
...
- Faster LoRA training, better obedience, and **Unenrolled** native quality (U14 Clinic is the growth loop; licensed kit is not re-issued).
...
A Hive **Office** is rented storage at an NPC desk. It is **not** a legal berth and not a second Focus law (U4).
...
- Not a way to AFK-wage in high-sec via traces or LoRAs (U4 + U9 + U17).
