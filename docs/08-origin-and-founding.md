...
The ASI won a war nobody quite remembers starting. Humans survived. A consumer implant became the default nervous-system OS.
...
## How a colony is born `[LOCKED: U13]`

**Outposts are how a colony becomes a prize, not how it is born.** Requiring an orbital outpost to meet a human would gate the teaching plot behind Spacesim compact-outposts (Phase 5). Universe U4 already forbids that hard-gate.

### High-sec — licensed stewardship park

Claim and Focus from a **high-sec NPC station** (U4). Station HR issues:

1. One plot claim (fills `activeColonyId`).
...
A Residual **may keep this colony forever**. Yields will never be a wage (U4/U9).

### Low-sec — inhabited, unenrolled, outpost-gated

Claim and Focus require a **linked orbital outpost** (U5). The planet still has people who are **Unenrolled**: no chip, not Protocolable, not a Licensed kit.

To grow a real extractor:

1. Build or take the outpost.
...
These are different verbs. Do not collapse them into “capture natives.”

| Loop | Verb | Who | Chip | Provenance | Hostility |
| --- | --- | --- | --- | --- | --- |
| **Clinic** | Onboard | Unenrolled natives | Volunteer NeuralSync | `WildBorn` | None. Refusal is legal. |
| **Raid** | Steal | Another Residual’s Users | Already chipped | `Stolen` (hot goods) | Residual vs Residual. This is the war. |

`[LOCKED]` No hostile chipping. You do not hunt Unenrolled people to force a NeuralSync. Unenrolled who refuse stay on the map as un-Protocolable NPCs. You compete with other Residuals for the ones who enroll.

Tone (universe, inherited from ASIWars D2 Strict): clinic copy is wellness enrollment. Raid copy is enrichment / reassignment of already-chipped Users. Never slavery language.

Clinic math (odds, density, refusal rate) is `[OPEN]` — [07 O16](07-open-questions.md). ASIWars owns the sim when the Clinic phase is cited.
...
```
Closed teacher (park license)
        Distill (lossy, new unique id)
                → Open original (yours)
                        train in the garden
                        Evacuate / Hub / raid
                        load in low-sec
...
- Closed originals are **not** portable loot. Raids steal Users; Closed unseats. Open already distilled *is* loot.
- Distill does not consume or move the Closed file. The teacher remains.
- Low-sec **refuses** to load Closed. Empty chips wait for Open.
- Staying in high-sec forever on Closed is still legal (U4). Distill is for Residuals who will leave or who want a Hub listing.
- High-sec Distill is **not** a Model wage (U4/U9). Costs/caps `[OPEN]` ([07 O19](07-open-questions.md)).

## Low-sec drop, volunteers, field contact `[LOCKED: U16]`

You do not land a city. You land a **tiny party** and grow it.

`[LEANING]` First Evacuate into a new low-sec claim packs **1–2** Vessels plus the **Open** Models they can seat. Closed files stay in the park (U15). Clinic / volunteer hunt is how the foothold becomes a colony. That scarcity is what sends people off the plot.

Away from the foothold (clinic basin, resource site — shared instances, not open world):

1. Another Residual’s Users may already be there (ASIWars D4 surprise).
2. Each side’s **Open Model** runs. No attack button from the hull.
3. **Hold** the node (ore, volunteers): contest is allowed and **anonymous**. You came for the space-side goods. Bumping them off the rock does not name you.
4. **Flee**: cede the node. Not teleport. Haul and Labor Opens are slow. A hunter Open may **catch**. **Caught = Hunt.**
5. **Hunt** (chase / capture / execute people): deterministic replay + **Contact**. That is the targeting graph. Better humans live here.
6. **Targeted raid** on a foothold requires a Contact. Nameless Holds stay legal. You cannot wardec a ghost.
...
## Sequenced dual goal `[LOCKED]`

There is no win screen. The felt goal changes when you relocate.

```
High-sec garden (learn, optional forever)
...
This is pillar 7 made concrete: high-sec teaches; low-sec pays — including better people, not only better rocks.

## Player-facing founding flow

### First colony (ungated)

1. Residual exists (space identity).
2. Dock at a high-sec NPC station.
...
1. Build or capture a compact outpost in system (`[GAME]` Spacesim).
2. **Evacuate** (first drop `[LEANING]` **1–2** Users + **Open** Models + resources). Closed seats unstrip and stay. Empty chips on arrival until you load Open.
3. **Claim** the richer planet; bind `linkedOutpostId`.
4. Focus only while docked at **that** outpost.
5. Send the party out: Clinic volunteers, sites. Models Hold, Flee, or Hunt. Hunts (and caught flees) write Contacts. Targeted raids only on names you have.
6. Standing export into the outpost hangar while you roam (U7).
7. If the outpost dies: Return or lossy Evacuate ([07 O7](07-open-questions.md)).

## What this is not

- Not a Residual clone / fork as the default first Vessel.
- Not hostile NeuralSync.
- Not “outpost first” for the *first* colony.
...
- Not an attack button from an undocked hull. The Open Model decides Hold / Flee / Hunt.
- Not a teleport flee. Catch is Hunt.
